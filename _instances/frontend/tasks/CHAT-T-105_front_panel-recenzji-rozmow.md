...
**iter.3 (2026-06-29) — REWIZJA ADR-102 D3: `nowy` = skrzynka katalogu** (feedback Karola: domyślny widok był pusty, bo kolejka liczyła tylko istniejące wiersze, a auto-flag P15c nie istnieje). Backend (`ConversationReviewRepository`): `listByStatus('nowy')` + `countsByStatus()` listują/liczą rozmowy BEZ wiersza recenzji (LEFT JOIN) + jawne `nowy`, sort po `started_at` DESC. Oznaczenie dowolnym statusem usuwa rozmowę ze skrzynki `nowy`. Front: default `resolveReviewFilter()` zmieniony `do_weryfikacji`→`nowy`. Testy real-path Railway: 35/35 (repo, +5 inbox) + 13/13 (counts, niezmiennik przepisany na „oś robocza = wiersze, nowy = skrzynka"). Wymaga redeploy standalone (ADR-089) + re-upload modułu. Szczegóły: ADR-102 „REWIZJA D3".

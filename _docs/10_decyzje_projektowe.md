...
**NOTA NR 2 (2026-07-28) — REWIZJA noty 93a: fundament faktyczny był fałszywy, `visibility` WRACA jako kryterium (złożone z `afo`).** Karta Chat - 69 (conv 589, produkt 4287 Explorer 260m pokazany klientowi mimo `vis='none'`). Powód rewizji, nie dostrojenia: fundament noty 93a — „Luigi's Box ignoruje `visibility`, dowód `szukaj?s=Torba MARES Cruise`→produkt 3920" — **obalony pomiarem na żywo tego samego dowodu**. Test 2026-07-28 (przeglądarka + network requests, potwierdzone że `/szukaj` woła `api.luigisbox.tech`, nie fallback PrestaShop): zapytanie dokładną nazwą **4 z 4** produktów `vis='none'` (4270, 4682, 5256 — i **ten sam 3920** z oryginalnego dowodu) **nie zwraca ich w wynikach** — Luigi's Box podstawia inny, podobny produkt. Kontrola po stronie `afo=0`+`vis='both'` (1629 POSEIDON Xstream Duration90) zadziałała zgodnie z ADR-123 — ten filar zostaje.

**Ustalenie Karola (2026-07-28): `visibility='none'` w tym sklepie oznacza „produkt nieprodukowany".** Docelowo rozstrzygał to będzie moduł **HADES** (`Hades_PS`, w budowie na środowisku deweloperskim) — będzie bazował na `visibility`, ale doda mechanizm świadomego przenoszenia produktów do stanu „Hades" (nie wyszukiwalne, linki nie giną) i wygodny przegląd. Do czasu wdrożenia HADES-a `visibility='none'` jest najlepszym dostępnym sygnałem tego samego zjawiska, które `available_for_order=0` łapie tylko częściowo (13 aktywnych produktów `vis='none'`+`afo=1` na PROD, w tym realne pozycje wysokiej wartości: 4270 konsola APEKS, 4682 jacket TUSA, 5256 kurtka SANTI, + 4287 z karty).

**Decyzja 1 (Chat-69, 1a) — kryterium złożone.** `ProductSearch` wyklucza z rekomendacji gdy `available_for_order=0` **LUB** `visibility='none'` (zamiast samego `afo`). Sprawdzone: root-category (`id_category_default IN (1,2)`) jako dodatkowy sygnał **odrzucony** — 43 aktywnych produktów ma root, 35 z nich to zwykłe `both/afo=1`; złapałby dobre produkty. Kompozyt `afo/vis` wystarcza bez niego.

**Decyzja 2 (Chat-69) — carve-out nawigacyjny ZOSTAJE, ale z inną narracją niż `afo=0`.** Klient pytający wprost (intent=navigational + exact_keywords) o produkt `vis='none'`+`afo=1` ma dostać odpowiedź — produkt jest realnie zamawialny (koszyk działa), więc różni się to od `afo=0` (koszyk zablokowany). Nowa flaga w wyniku narzędzia: `no_longer_manufactured: true` (odróżniona od `available_for_order: false`). SystemPrompt: „nie jest już produkowany, ale u nas nadal dostępny do zamówienia" + opcjonalna propozycja alternatywy — NIGDY „nie da się zamówić" dla tego statusu.

**Decyzja 3 (Chat-69) — nie zakładam karty Sklep.** Karol wyjaśnił znaczenie `visibility='none'` wprost (nieprodukowany) i zapowiedział HADES jako docelowe rozwiązanie po stronie katalogu — nie ma otwartego pytania do przekazania. Rewizja ta jest tymczasowa do czasu wdrożenia HADES-a; wtedy do ponownej oceny, czy kompozyt `afo/vis` zastąpić bezpośrednim odczytem stanu „Hades".

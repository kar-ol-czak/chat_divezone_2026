...
# Wersja: 3.19 | Data: 2026-05-26 (T-022b DONE: kalibracja AI_MAX_TOKENS=2500 + diagnoza cache OpenAI H5 stochastic, hipoteza "max blokuje cache" obalona empirycznie macierzą; commit 5205f4f)
...
| T-024 | red-team Faza 1 orchestrator: Promptfoo + custom HTTP provider na DEV endpoint + warstwy oceny W0 regex + W1 sedzia (gpt-5.4) + integracja domain_rules + judge_prompts/ (W1 + W2 panel) + meta-eval golden set (50-100 transkryptow, Cohen kappa >=0.7) + snapshot ground truth (czeka na port 14368 Railway) | P1 | po T-023 (gotowe) — spec gotowa w `_instances/integration/tasks/T-024_integration_redteam-orchestrator.md` |

...
- **Ranking par lewe/prawe (bliźniacze produkty) jest niestabilny z dnia na dzień.** Przy 4 niemal identycznych produktach (np. szkła MC211 L/P, BF211 L/P) kolejność RRF waha się po nocnym odświeżeniu embeddingów/enrichment. Rozmowa 833 (05:27 UTC): 6993 poza top-5. Pomiar tego samego dnia po odświeżeniu: 6993 na pozycji 4, `limit=5`, deterministycznie 3/3 próby. Wniosek: przy diagnozie „produkt wypadł z wyników" sprawdź stan RANKINGU w momencie rozmowy (`search_diagnostics`), nie tylko bieżący — mogą się różnić. Otwarty problem, obserwowany (ADR-135/136).
- **Model potrafi orzec o wariancie WBREW tool_result.** Rozmowa 833: `get_product_combinations` zwróciło 6994 (BF211 prawe) z +3,5 i 6573 (MC211 prawe) BEZ +3,5, bot napisał odwrotnie. `tools_used` zawiera wywołanie ≠ model użył wyniku. Przy weryfikacji odpowiedzi o wariantach porównuj treść odpowiedzi z tool_result po `product_id`, nie ufaj samej obecności wywołania w `tools_used`. Naprawa: ADR-136 / CHAT-T-170.

- **`cache_creation_tokens` bywa płacone DWA RAZY na rozmowę i nie jest to wina promptu.** Włączony `thinking` (panel `reasoning_effort` ≠ puste → `budget_tokens`, u nas `minimal`=1024) sprawia, że Anthropic dokłada ~22-tokenowy wstrzyk **przed blokiem narzędzi**, ale TYLKO gdy ostatnia wiadomość jest od użytkownika; po `tool_result` wstrzyku nie ma. Powstają dwa różne prefiksy cache, każdy z własnym zimnym zapisem (~51,5 tys. tok). Zmierzone sondą: z thinking 51577 vs 51555, bez thinking oba 51555; na PROD +22 w 292/323 wywołań pierwszych w turze i 0 w 315/320 wywołań w pętli narzędzi. **Żadne ustawienie breakpointów tego nie scali** — wstrzyk leży przed narzędziami (segment samych narzędzi też rozjeżdża się o 22: 6689 vs 6667). To 47% wszystkich zapisów cache. Dowód: CHAT-T-176, pomiar 2026-07-27. Naprawa: ADR-139 / CHAT-T-177 (adaptive thinking na Sonnet 5 — zachowuje breakpointy; `budget_tokens` jest deprecated na 4.6 i zwraca 400 na Sonnet 5).
- **Zmienna wysoko w system prompcie kosztuje CAŁY ogon, nie samą siebie.** Cache Anthropic działa na prefiksie: data w linii 61 promptu liczącego ~45 tys. tokenów unieważniała wszystko za sobą przy każdej zmianie doby, a kontekst chipów — przy każdej rozmowie z chipami. Naprawa (CHAT-T-176): `SystemPrompt::buildStatic()` + `buildVolatile()`, dwa bloki `system`, `cache_control` na ostatnim **cacheable**. Zweryfikowane: po zmianie zmiana doby i doklejenie chipów dają `write=0, read=51562`. **Przy dopisywaniu czegokolwiek do promptu pytaj najpierw: czy to zmienia się częściej niż deploy?** Jeśli tak — do `buildVolatile()`, nie do korpusu.
- **TTL cache 1h nie jest darmowym ulepszeniem.** Zapis 1h kosztuje 2× zamiast 1,25×, więc opłaca się dopiero przy gęstym ruchu. Profil divezone (30 dni): odstępy między rozmowami <5 min 20%, 5min–1h 33%, >1h 47% → trafienia rosną z 20% na 54%, ale rachunek wychodzi 30,2M vs 31,1M jednostek, czyli **w granicach błędu**. Zostaje 5 min. Dowód: CHAT-T-176, `divechat_message_usage` 2026-06-27…07-27.

- **Restart monitora Railway ma DWIE różne przyczyny i mylą się nawzajem.** `guard.log` rozróżnia je jawnie: `ZAMARCIE` = proces żył, ale log stał >60 s (objaw SIECIOWY), `proces martwy` = proces padł, w praktyce `Fatal error: Maximum execution time of 30 seconds exceeded` w `monitor_nohup.out`. Rozkład na dobę: „proces martwy" 2-3 KAŻDEGO dnia niezależnie od stanu łącza, `ZAMARCIE` **wyłącznie w dobach z epizodem** (2026-09-02: 39 ZAMARCIE + 1 martwy; 2026-08-27: 20 + 1; doby czyste 08-22…09-01, 09-03, 09-04: 0 ZAMARCIE, 2-3 martwy). **33 nagłówki `# metryki:` w logu 2026-09-02 to były ZAMARCIA, nie fatale.** Konsekwencja pomiarowa: w epizodzie monitor traci do 60 s na każde zamarcie (2026-09-02 do ~39 minut ślepych w oknie 1h42m), więc liczby FAIL z dób epizodowych są **zaniżone**, a `max przerwa` w oknach raportu (np. 474 s) to zamarcie plus czas wskrzeszenia, nie ciągły pomiar. Dowód: `guard.log` + `monitor_nohup.out` (130 fatali / 142 „proces martwy" narastająco), pomiar 2026-09-04, CHAT-T-183.
- **`pgrep -f 'railway_monitor[.]php'` łapie własną komendę, jeśli gdziekolwiek w niej pada literał ścieżki.** Wariant nawiasowy chroni tylko przed dopasowaniem samego wzorca — gdy ta sama komenda zawiera np. `md5sum /home/divezone/_diag/railway_monitor.php`, cała linia `bash -c` pasuje i `pgrep` zwraca dwa PID-y. Przy `pkill` oznacza to ubicie własnej sesji SSH. Sprawdzaj liczbę procesów przez `ps -o pid,etime,cmd -p $(cat ~/_diag/railway_monitor.pid)`, nie przez `pgrep` w komendzie mieszanej. Dowód: pomiar 2026-09-04, dwa PID-y (2044435 realny, drugi to własny `bash -c`), CHAT-T-183.

- **`mtr` JEST na serwerze, mimo że CHAT-T-119 zapisał „mtr BRAK — NIE używać".** Pomiar 2026-09-09 13:09 UTC: `/usr/sbin/mtr` (0.92) istnieje, `getcap /usr/sbin/mtr-packet` → `cap_net_raw=ep`, więc działa z konta `divezone` **bez roota**, i dochodzi do hopa 11 (`66.33.22.230`), którego traceroute nigdy nie pokazywał. Wołaj **pełną ścieżką** `/usr/sbin/mtr`. Uwaga na wyjaśnienie tej pomyłki: wersja „bo `/usr/sbin` nie jest w PATH" **nie odtwarza się** — 2026-09-09 PATH konta i PATH w `shell_exec()` z PHP zawierają `/usr/sbin`, a `command -v mtr` zwraca `/usr/sbin/mtr`. Zmierzony jest sam fakt dostępności, nie jego przyczyna (CHAT-T-185).

- **`mtr` z pełną ścieżką NIE wystarcza: `mtr` odpala `mtr-packet` PO NAZWIE, przez PATH.** Monitor jest wskrzeszany przez guard z crona i ma `PATH=/usr/bin:/bin` (odczyt z `/proc/<pid>/environ`, 2026-09-10), a `mtr-packet` leży w `/usr/sbin` — w `/usr/bin` go nie ma. Skutek: funkcja wdrożona w CHAT-T-185 była **martwa od wdrożenia**, `mtr_baseline_20260909.txt` zawierał wyłącznie `Failure to start mtr-packet: Invalid argument` (oba przebiegi, blok 0 s). Dowód pary: `env -i PATH=/usr/bin:/bin` → błąd, `env -i PATH=/usr/sbin:/usr/bin:/bin` → pełny raport do hopa 11; kontrola `MTR_PACKET=/usr/sbin/mtr-packet` pod cronowym PATH też naprawia. **Testowanie takich rzeczy z powłoki interaktywnej niczego nie dowodzi** — ma `/usr/sbin` w PATH i przepuszcza błąd (tak zawiodła weryfikacja T-185). Testuj `env -i PATH=/usr/bin:/bin` (CHAT-T-186).

- **`command -v ea-php84` wskazuje DWIE RÓŻNE binarki zależnie od PATH — pod cronem php-cgi, interaktywnie php.** `/usr/bin/ea-php84` → `/opt/cpanel/ea-php84/root/usr/bin/php-cgi` (SAPI `cgi-fcgi`, `max_execution_time=30`, nie zna nawet `-r`), `/usr/local/bin/ea-php84` → `.../usr/bin/php` (SAPI `cli`, `max_execution_time=0`). Cron ma `PATH=/usr/bin:/bin`, więc `command -v` w skrypcie cronowym wybiera CGI, a ten sam skrypt uruchomiony ręcznie wybiera CLI i „działa". Tak monitor Railway chodził od początku pod php-cgi i zbierał 139 wpisów `Maximum execution time of 30 seconds exceeded` (2-3 zgony na dobę, wpis `proces martwy` w `guard.log`). Dowód pary (2026-09-14, produkcja): `env -i PATH=/usr/bin:/bin bash -c 'command -v ea-php84'` → `/usr/bin/ea-php84`, interaktywnie → `/usr/local/bin/ea-php84`. **Do PHP CLI wołaj `/usr/local/bin/ea-php84` pełną ścieżką, nigdy przez PATH**; dowodem w logu jest `sapi=cli` (CHAT-T-190; skutek leczył CHAT-T-189, przyczynę T-190). Ta sama rodzina błędu co `mtr-packet` wyżej: **cronowy PATH to inne środowisko niż Twoja sesja SSH**.

- **`rsync -a` na `~/_diag/railway_monitor_guard.sh` WYŁĄCZA guarda: repo ma plik `644`, produkcja `755`.** Cron woła skrypt bezpośrednio (`* * * * * /home/divezone/_diag/railway_monitor_guard.sh`), więc bez bitu wykonywalności guard nie startuje w ogóle, a objaw wyjdzie dopiero przy pierwszym zgonie monitora — czyli po godzinach ciszy, wyglądającej jak zdrowie. `-a` implikuje `-p`, które zsynchronizuje prawa z repo. **Deployuj `rsync --no-perms` i sprawdzaj `ls -la` po transferze** (`-rwxr-xr-x`). Dowód: `git ls-files -s` → `100644`, produkcja przed i po wdrożeniu → `-rwxr-xr-x`, CHAT-T-190 (2026-09-14).

- **`connect_timeout` w DSN pdo_pgsql NIE JEST honorowany. Działa wyłącznie `PDO::ATTR_TIMEOUT`.** pdo_pgsql dokleja własne `connect_timeout=<ATTR_TIMEOUT, domyślnie 30>` na **końcu** conninfo, a libpq przy powtórzonym słowie kluczowym honoruje **ostatnie** wystąpienie — wartość wpisana wcześniej w DSN jest nadpisywana. Pomiar na produkcji (PHP 8.4.24, libpq 13.23, cel `192.0.2.1` = TEST-NET-1, bez trasy): sam DSN `connect_timeout=2` → **7088 ms** (pomiar architekta) i **7108 ms** (pomiar CC, ten sam cel), `PDO::ATTR_TIMEOUT=2` → **2002 ms**, kontrola dodatnia `pg_connect` z tym samym conninfo → **2002 ms**. Kontrola, że DSN w ogóle dochodzi do libpq: wstrzyknięty `bogus_keyword=1` daje `invalid connection option "bogus_keyword"` — parametr nie jest ignorowany, tylko **nadpisywany**. **Skutek: deklarowana od CHAT-T-107 „szybka degradacja w ~2 s" nigdy nie działała** — realny czas degradacji przy awarii Railway wyznaczał timeout TCP (lokalnie, przy celu milcząco gubiącym pakiety: 30 021 ms, czyli domyślne 30 s libpq). Każda wcześniejsza ocena czasu degradacji czatu opierała się na tej nieprawdziwej liczbie. Naprawa: `PostgresConnection::getPdo()`, CHAT-T-191 (2026-09-15).
- **`PDOException::getCode()` przy błędzie CONNECTU zwraca natywny kod pgsql (`int 7`), nie SQLSTATE.** SQLSTATE siedzi w `errorInfo[0]`. Dowód (produkcja, celowo złe hasło, jedna próba): `getCode() = 7` (typ `integer`), `errorInfo = ["08006", 7, "FATAL:  password authentication failed for user \"postgres\""]`. **Konsekwencja: gałęzie klasyfikatora oparte na `'08001'` / `'08004'` przy błędach connectu nie strzelają NIGDY** — nierozpoznany błąd wpada w `'7'` i jest traktowany jak zerwanie (SOFT, 3 próby, komunikat „spróbuj za moment"), więc błąd uwierzytelnienia jest dziś ponawiany i raportowany klientowi jako przejściowy. Dla błędów fazy ZAPYTANIA PDO wstawia SQLSTATE do kodu wyjątku i tam te gałęzie działają. Stan sprzed CHAT-T-191, nie regres; pomiar i opis: CHAT-T-191 (2026-09-15).

...
| `knowledge_gap` | bot nie znalazł odpowiedzi | **ZEPSUTE dla `search_products`** (naprawa: ADR-126 / CHAT-T-148). Próg 0,5 (skala cosine) porównywany z `rrf_score` (sufit zmierzony **0,1230**) → **zawsze `true`**. Dowód empiryczny: **0 z 1605** pozycji na produkcji osiągnęło 0,5. Do tego **sticky** (`ConversationStore:189`: `? OR COALESCE(...)`) — raz zapalona nie gaśnie. PROD: **237 rozmów** z `search_products`, **237 z flagą** `true` → filtr „Luki wiedzy" nie filtruje niczego. Po naprawie (reguła 128b: `gap = zero wyników`) panel: **339 → 196** globalnie. Migracja rusza **tylko 191** rozmów z pełną diagnostyką; **86** (migawka) + **94** (brak diagnostyki) zostają nietknięte — nieprzeliczalne | sekcja 4, ADR-126 + noty 1-2, decyzje 128b/130b/132a |
...
- **`search_diagnostics` to MIGAWKA ostatniej tury, nie historia rozmowy.** Nadpisywana przy każdej turze, a `knowledge_gap` jest sticky → przeliczanie historii z diagnostyki **gubi sygnał z wcześniejszych tur**. Dowód: 86 z 277 rozmów ma diagnostykę uboższą niż `messages` (200 wywołań bez śladu), ADR-126 nota nr 1.

...
**⚠️ `composer dump-autoload -o` przepisuje pliki vendora, które TEŻ są monitorowane.** Jeśli deploy zawierał NOWĄ klasę PHP (i odpaliłeś `dump-autoload`), do bloku deklaracji Sentinela DODAJ oba pliki: `vendor/composer/autoload_classmap.php` i `vendor/composer/autoload_static.php` (obok właściwych plików deployu). Inaczej alert `PHP_FILE_CHANGED` o vendorze zostanie mimo zadeklarowania reszty. Dotyczy tylko deployów z nową klasą — sama edycja istniejącego pliku nie rusza autoloadera. Dowód: CHAT-T-151 (nowa klasa `InternationalShipping`) wygenerował alerty o obu plikach vendora.
